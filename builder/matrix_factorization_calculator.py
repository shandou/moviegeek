...
class MatrixFactorization(object):
    ...
    def initialize_factors(self, ratings, k=25):
        ...
        self.item_counts = (
            ratings[["movie_id", "rating"]].groupby("movie_id").count()
        )
        self.item_counts = self.item_counts.reset_index()

        self.item_sum = (
            ratings[["movie_id", "rating"]].groupby("movie_id").sum()
        )
    ...
    def predict(self, user, item):
        ...
        b_ui = (
            self.all_movies_mean + self.user_bias[user] + self.item_bias[item]
        )
        prediction = b_ui + pq
    ...
    def split_data(self, min_rank, ratings):
        ...
        test_temp = ratings[ratings["user_id"].isin(test_users)].sort_values(
            "rating_timestamp", ascending=False
        )
    ...
    def meta_parameter_train(self, ratings_df):
        ...
        for k in [15, 20, 30, 40, 50, 75, 100]:
            ...
            for factor in range(k):
                ...
                while not finished:
                    train_mse = self.stocastic_gradient_descent(
                        factor, index_randomized, ratings
                    )
                    ...
                    self.log(
                        str(k),
                        f"{factor}, {iterations}, {train_mse}, {test_mse}, {datetime.now() - factor_time}",
                    )
    ...
    def calculate_rmse(self, ratings, factor):
        def difference(row):
            ...
            pq = np.dot(
                self.item_factors[item][: factor + 1],
                self.user_factors[user][: factor + 1].T,
            )
            b_ui = (
                self.all_movies_mean
                + self.user_bias[user]
                + self.item_bias[item]
            )
            prediction = b_ui + pq
    ...
    def train(self, ratings_df, k=40):
        ...
        self.initialize_factors(ratings_df, k)
        self.logger.info(
            "training matrix factorization at {}".format(datetime.now())
        )
        ...
        for factor in range(k):
            ...
            self.logger.info(
                "finished factor {} on f={} i={} err={}".format(
                    factor,
    ...
    def finished(
        self, iterations, last_err, current_err, last_test_mse=0.0, test_mse=0.0
    ):

        if (
            last_test_mse < test_mse
            or iterations >= self.MAX_ITERATIONS
            or last_err - current_err < 0.01
        ):
            self.logger.info(
                "Finish w iterations: {}, last_err: {}, current_err {}, lst_rmse {}, rmse {}".format(
                    iterations, last_err, current_err, last_test_mse, test_mse
                )
            )
            return True
        else:
    ...
    def save(self, factor, finished):
        ...
        user_bias = {
            uid: self.user_bias[self.u_inx[uid]] for uid in self.u_inx.keys()
        }
        item_bias = {
            iid: self.item_bias[self.i_inx[iid]] for iid in self.i_inx.keys()
        }
...
if __name__ == "__main__":
    ...
    MF = MatrixFactorization(
        save_path="./models/funkSVD/{}/".format(datetime.now()),
        max_iterations=40,
    )
